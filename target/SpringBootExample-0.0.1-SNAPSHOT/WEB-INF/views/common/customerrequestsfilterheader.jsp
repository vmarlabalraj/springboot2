...
<div class="filter-container">
	...
	<div class="filter-col filter-field-length">
		<label>From</label>
		<input type="text" id="fromDate" name="d1" class="form-control txt-field" value="${filterData.custReqStartDate}" />
	</div>
	<div class="filter-col filter-field-length">
		<label>To</label>
...
<div class="filter-container">
	<div class="filter-col filter-field-length">
		<label>Status</label>
		<select id="statusIdDD" name="status" onchange="setFilterStatusByDD()"
			class="btn selectric_dd cssp btn-default dropdown-toggle">
			<option value="">ALL</option>
			<c:forEach var="status" items="${statusMap}">
				<c:choose>
				    <c:when test="${filterData.status == status.key}">
	...
	<div class="filter-col filter-field-length">
		...
		<select id="adminUserIdDD" name="assignedTo" onchange="setFilterAssignedToByDD()"
			class="btn selectric_dd cssp btn-default dropdown-toggle">
			<option value="">ALL</option>
			<c:forEach var="adminUser" items="${adminUsersMap}">
				<c:choose>
				    <c:when test="${filterData.assignedTo == adminUser.key}">
					        <option value="${adminUser.key}" selected="selected">${adminUser.value}</option>
				    </c:when>
				    <c:otherwise>
					        <option value="${adminUser.key}">${adminUser.value}</option>
				    </c:otherwise>
				</c:choose>
			</c:forEach>
		</select>
...
function clearFilters(){
	...
	$('#statusDD').prop('selectedIndex',0);
	$('#statusDD').selectric('refresh');
	
	$('#adminUserIdDD').prop('selectedIndex',0);
	$('#adminUserIdDD').selectric('refresh');
